...
- **Navigation context**: the launcher writes the current form context to localStorage on every OnLoad without contacting the agent. The pane adds a trusted context envelope to each user prompt and never sends a separate `pvaSetContext` event, preventing context synchronization from creating a second agent turn.
